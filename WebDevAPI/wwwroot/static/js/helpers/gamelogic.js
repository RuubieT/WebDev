...
async function createGame(){
    ...
    let createdGame = await postData('api/Pokertable/Create', createData);
...
async function getPlayers(){
    ...
    let players = await getData(`/api/Pokertable/GetPlayers/${table}`);
    if(players){
        console.log(players);
    }
}
...
async function getHand(){
    ...
    const username = getCookie("username");
    let hand = await getData(`/api/Pokertable/Hand/${username}`);
    if(hand){
        var firstCard = new Card(SUITS[hand.firstCard.suit], CARD_VALUE_MAP[VALUES[hand.firstCard.value]]);
        var secondCard = new Card(SUITS[hand.secondCard.suit], CARD_VALUE_MAP[VALUES[hand.secondCard.value]]) 
    ...
    if(cards){
        ...
        if (!tablecards) { 
            ...
            for(const i in cards){
                var tablecard = new Card(SUITS[cards[i].mySuit], CARD_VALUE_MAP[VALUES[cards[i].myValue]]);
